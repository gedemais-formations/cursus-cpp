...
int create_board(t_board **pBoard, int number_row, int number_col, int case_size) {
    ...
    *pBoard = malloc(sizeof(t_board));
    ...
    (*pBoard)->board = malloc(sizeof(bool) * number_col * number_row);
    if((*pBoard)->board == NULL) return print_error(ERROR_CANT_ALLOC_MEMORY);
...
int init_board(t_board *pBoard, unsigned short saturation) {
    ...
    pBoard->board = malloc(sizeof(char) * pBoard->case_col * pBoard->case_row);
    if(pBoard->board == NULL) return print_error(ERROR_CANT_ALLOC_MEMORY);

    for (int i = 0; i < pBoard->case_row; ++i) {
        for (int j = 0; j < pBoard->case_col; ++j) {
            set_case(pBoard->board, pBoard->case_col, i, j, rand()%100 < saturation);
            //printf("%d", get_case(pBoard->board, pBoard->case_col, i, j));
        }
        //printf("\n");
    }
...
bool* copy_board(t_board board) {
    ...
    copy = memcpy(copy, board.board, tab_size);
...
bool willBeAlive(t_board board, int row, int col) {
    ...
    for (int i = row-1; i <= row+1; ++i) {
        for (int j = col-1; j <= col+1; ++j) {
            ...
            if(i>=0 && i<board.case_row && j>=0 &&  j < board.case_col && (i!=row || j!=col )) {
                case_val = get_case(board.board, board.case_col, i, j);
            }
    ...
    switch (neighbor_val) {
        case 3:
            return true;
        case 2:
            return get_case(board.board, board.case_col, row, col);
        default:
            return false;
    }
}
...
void set_case(bool *edited_board, int lineWidth, int row, int col, bool value) {
    edited_board[row * lineWidth + col]= value;
}